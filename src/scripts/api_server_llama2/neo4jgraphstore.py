...
from graphstorebase import GraphStore
...
match_query = """
match p = (n:ENTITY)-[r]->(b) where n.name='qname' return p
"""
...
class Neo4jGraphStore(GraphStore):
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ValueError(
                ...
                "Please ensure the APOC plugin is installed"
            )
    ...
    def retrieve(self, entity:str) -> List[List[str]]:
        """召回知识"""
        # 或者说 RETURN n.name, r.name, b.name?
        query = """
            MATCH p = (n:ENTITY)-[r]->(b)
            WHERE n.name = $entity
            RETURN n.name, r.name, b.name;
        """
        
        prepared_statement = query
        with self._driver.session(database=self._database) as session:
            data = session.run(prepared_statement, {"entity":entity})
